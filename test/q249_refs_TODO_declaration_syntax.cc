...
int main() {
    int a = '0';
    char const &b = a;                      // char const reference to a - use b as you would a but its const
    //char const b = &a;                    // error: cannot initialize a variable of type 'const char' with an rvalue of type 'int *'
    //char const *c = a;                    // error: cannot initialize a variable of type 'const char *' with an lvalue of type 'int'
    //char const *c = &a;                   // error: cannot initialize a variable of type 'const char *' with an rvalue of type 'int *'    
    //char const *c = (char const *)a;       // explicit cast of int * to char const * - unsurprisingly causes segmentation fault
    //char const *c = (char const *)&a;       // explicit cast of int * to char const *
                                            // I thought this would cast the address of a - &a to (char const *)
                                            // but c behave like a reference - so something not right about this syntax
    char const *c = (char const *)(&a);

    cout << "&a:" << &a << endl;
    cout << "&a = &*c:" << &*c << endl;     // 0
    //cout << "&b:" << &b << endl;            // undefined? 0 followed by gibberish: &b:0o1??? 
    cout << "-"  << endl; 
    cout << "a:" << a << endl;              // 48 - ascii val - int
    cout << "&a:" << &a << endl;            // 0x7ffee37b717c   address of a (pointer to a)
    cout << "*&a:" << *&a << endl;          // *a meaningless a is not as pointer - but *(&a) = 48
    cout << "b:" << b << endl;              // 0 - char const
    cout << "c:" << c << endl;              // 
    cout << "(char)a:" << (char)a << endl;  // 0 - cast to char
    a++;                                    // increment int
    cout << "a:" << a << endl;              // 49 - ascii val int
    cout << "(char)a:" << (char)a << endl;  // 1 - cast to char
    cout << "b:" << b << endl;
    cout << "&*c:" << &*c << endl;          // 1
    cout << "*&c:" << *&c << endl;          // 1
    cout << "c:" << c << endl;              // 1 - char const * - still points at a - it is the pointer that is const
    cout << "*c:" << *c << endl;            // 1 
    cout << "&c:" << &c << endl;            // 0x7ffeee227160 - where char const * points < INCORRECT address of pointer? how to get pointer value?
    c++;
    cout << "&c:" << &c << endl;            // 0x7ffeee227160 - char const * - the pointer is const cannot be incremented - dont you love C++
    cout << "c:" << c << endl;              //
    cout << "*c:" << *c << endl;            //
}

// TODO
// c & *c appear to have same output
// sometying not quite right about line 98 - char const *c = (char const *)&a;
// c behaving like reference
