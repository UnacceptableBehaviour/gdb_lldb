...
int main() {
  ...
  //char* a = "Hello"; // warning: conversion from string literal to 'char *' is deprecated [-Wc++11-compat-deprecated-writable-strings]
  //const char* a = "Hello"; // error: read-only variable is not assignable    a[4] = '\0';
  //                                                                         //~~~~ ^
...
// Modifying string literals is undefined behavior. In practice, the implementation can
// for instance store the string literal in read-only memory, such as the code segment.
// Two string literals might even be stored in overlapping memory. So allowing them to be
// modified is clearly not a good idea.
