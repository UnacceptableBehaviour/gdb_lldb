...
template <typename T>
struct A {
    static_assert(T::value);     // only triggers if template instantiated
                                 // T::value  (T)class (::)scope (value)static variable name
};

struct B {
    static constexpr bool value = false;     // static var class scope
};

int main() {
    A<B>* pointer_to_template;   // warning: unused variable 'a' [-Wunused-variable]
                                 // doesn't instantiate template
    A<B> a;                      // instantiates template - test case
               // test_c++.cc:92:5: error: static_assert failed due to requirement 'B::value'
               //     static_assert(T::value);     // only triggers if template instantiated
               //     ^             ~~~~~~~~
               // test_c++.cc:103:10: note: in instantiation of template class 'A<B>' requested here
               //     A<B> a;    
