...
int main() { B b; }   // o/p abBA

// Base class ctor called then derived class on construction
// then reverse order when it goes out of scope -  here when program terminates


// EXPLANATION (see also delegation ctor below)
// The derived class constructor B() first calls the base class constructor A(), and then executes the
// compound-statement (the part inside {}) of its own body.
// 
// The derived class destructor ~B() first executes its own body, and then calls the
// base class destructor ~A().
// 
...
// the body, a destructor for class X calls (...) the destructors for X’s non-virtual direct base classes"
...
//    }
//    class_c(int my_max, int my_min) : class_c(my_max) { // execute class_c(my_max) first then class_c(int my_max, int my_min)
