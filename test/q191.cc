...
// more on C linkage - no overloading in C, therefore no name mangling, 
namespace A{
  extern "C" int x;             // C linkage ignores namespace
...
namespace B{
  extern "C" int x;     // same var as A::x
};

int A::x = 0;

int main(){
  std::cout << B::x;   // B::x same as A::x because of C linkage spec - prints 0
  A::x=1;
  std::cout << B::x;    // prints 1
}

// Due to the extern "C" specifications, A::x and B::x actually refer to the same variable.
// 
// x is first initialized to 0,
// then main() starts,
// 0 is printed,
// x is incremented to 1,
// and finally 1 is printed.
// 
