...
using Func = int();             // define function signature
                        // alias-declaration introducing the typedef-name Func
                        // for a function taking no parameters and returning an int.
struct S {
    Func f;                     // int f();     // declaration
};

int S::f() { return 1; }        // definition f in scope S::

int main() {
    S s;                        // declaration object s of type S
    std::cout << s.f() << std::endl;         // call f ret 1 o/p 1
}


// using Func = int() is an alias-declaration introducing the typedef-name Func for a
// function taking no parameters and returning an int.
// 
// This typedef-name can be used both for free functions and, as in this question, for member
// functions. §[class.mfct]¶8 in the C++ standard:
// 
// [Note: A member function can be declared (but not defined) using a typedef for a function type.
// The resulting member function has exactly the same type as it would have if the function declarator
