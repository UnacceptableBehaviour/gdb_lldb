...
//  
//  The type of a function is determined using the following rules. The type of each parameter (including function parameter packs)
//  is determined from its own decl-specifier-seq and declarator. (...) After producing the list of parameter types, any top-level
//  cv-qualifiers modifying a parameter type are deleted when forming the function type.
//  
...
//  Here the parameter types are "pointer to int" and "pointer to const int", respectively.
...
//  
//  Why is is the constness of parameters not part of the function type? When an argument is passed by value, a copy is made, and
//  the original argument is never modified anyway. So whether the parameter is const or not does not matter to the caller, it's
//  only relevant inside the function.
